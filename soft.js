			...
			if(rdata.data[i].name != 'php'){
				...
				if(rdata.data[i].price > 0){
					...
					if(rdata.data[i].end == '未开通' || rdata.data[i].end == '已到期' || rdata.data[i].end == '待支付'){
						handle = '<a class="btlink" onclick="payinstall(\''+rdata.data[i].title+'\')">立即购买</a>'
						titleClick = 'onclick="payinstall(\''+rdata.data[i].title+'\')" style="cursor:pointer"';
						if(rdata.data[i].end == '已到期') handle = '<a class="btlink" onclick="Renewinstall(\''+rdata.data[i].title+'\')">立即续费</a>'
					...
					if(rdata.data[i].end.indexOf('20') != -1 || rdata.data[i].end == '已到期') enddate = '<td class="c9 text-center">'+rdata.data[i].end+'<a class="btlink" onclick="Renewinstall(\''+rdata.data[i].title+'\')"> (续费)</a></td>';
...
function payinstall(pluginName,an){
	...
	$.post('/auth?action=get_plugin_price',{pluginName:pluginName},function(rdata){
		...
		if(rdata.status === false){
			...
			//付款
			var payhtml = '<div class="libPay pd20">\
			<div class="line"><span class="tname">开通时长</span>\
			<div class="info-r c4"><div class="btn-group" role="group">'+rdata+'</div></div></div>\
			<div class="line"><span class="tname">原价</span>\
			<div class="info-r c4"><s class="sprice" style="font-size: 14px;display: inline-block;line-height: 30px"></s>元/台</div></div>\
			<div class="line"><span class="tname" style="height:40px;line-height:40px">应付金额</span>\
			<div class="info-r c4"><span class="price"></span>元/台</div></div>\
			<div class="wxPay"><div class="wx-pic"></div><div class="ewm"></div>\
			<div class="wx-pay-info">使用 微信扫码 支付</div></div></div>';
		}
		layer.open({
			type: 1,
		    title: '开通' + pluginName,
		...
		if(rdata.status !== false && an !== undefined)  $(".btn-group .btn-success").click();
		...
		var timehwnd = setInterval(function countdown(){
			...
			if(imin == 0){
				...
				applyObj.attr("onclick","anTo('"+pluginName+"')");
...
//续费和升级专业版
function Renewinstall(pluginName){
	window.location.href = "/vpro"; return;
	var price = '<ul class="pay-btn-group">\
				<li class="pay-cycle-btn active"><span>3年付</span><em>5折</em></li>\
				<li class="pay-cycle-btn"><span>2年付</span><em>6.5折</em></li>\
				<li class="pay-cycle-btn"><span>年付</span><em>8折</em></li>\
				<li class="pay-cycle-btn"><span>半年付</span><em>9折</em></li>\
				<li class="pay-cycle-btn"><span>月付</span></li>\
				</ul>\
				<label class="bt-label">\
					<input class="bt-radio lib-coupon" type="checkbox" name="bt-checkbox">\
					<span class="bt-checkbox bt-radioinput"></span>使用专业版3年付代金券\
				</label>';
	var payhtml = '<div class="libPay" style="padding:30px">\
				...
				<div class="libPay-item f14 plr15">\
					...
					<div class="li-con c6">'+price+'</div>\
				</div>\
				<div class="lib-price-box text-center"><span class="lib-price-name f14"><b>总计</b></span><span class="price-txt"><b class="sale-price">356.4</b>元</span><s class="cost-price">712.8元</s></div>\
				<div class="paymethod">\
					<div class="pay-wx"><img src="ewm.png"></div>\
					<div class="pay-wx-info f16 text-center"><span class="wx-pay-ico mr5"></span>微信扫码支付</div>\
				</div>\
				<div class="paymethod-submit text-center" style="display:none">\
					<button class="btn btn-success btn-sm f16" style="width:200px;height:40px">提交</button>\
				</div>\
			</div>';
	layer.open({
		type: 1,
	...
	$(".pay-btn-group .pay-cycle-btn,.li-c-item li").click(function(){
		$(this).addClass("active").siblings().removeClass("active");
	});
	$(".bt-label").click(function(){
		if($(".lib-coupon").prop("checked")){
			$(".paymethod").hide();
			$(".paymethod-submit").show();
		}
		else{
			$(".paymethod").show();
			$(".paymethod-submit").hide();
		}
	})
}
//续费
function Renewinstall_old(pluginName){
	var loadT = layer.msg('正在获取插件信息,请稍候...',{icon:16,time:0,shade: [0.3, '#000']});
	$.post('/auth?action=get_plugin_price',{pluginName:pluginName},function(rdata){
		layer.close(loadT);
		if(rdata.status === false){
			...
			//付款
			var payhtml = '<div class="libPay pd20">\
			<div class="line"><span class="tname">续费时长</span>\
			<div class="info-r c4"><div class="btn-group" role="group">'+rdata+'</div></div></div>\
			<div class="line"><span class="tname">原价</span>\
			<div class="info-r c4"><s class="sprice" style="font-size: 14px;display: inline-block;line-height: 30px"></s>元/台</div></div>\
			<div class="line"><span class="tname" style="height:40px;line-height:40px">应付金额</span>\
		...
		if(rdata.status !== false)  $(".btn-group .btn-success").click();
		$(".btn-group .btn").click(function(){
			$(this).addClass("btn-success").siblings().removeClass("btn-success");
		});
...
function getRsCode(pid,price,sprice,cycle){
	$(".price").text(price);
	$(".sprice").text(sprice);
	var loadT = layer.msg('正在生成付款码,请稍候...',{icon:16,time:0,shade: [0.3, '#000']});
...
function anTo(pluginName){
	layer.closeAll();
	payinstall(pluginName,1);
}

//登陆宝塔官网帐户
function loginBT(pluginName){
	p1 = $("#p1").val();
	p2 = $("#p2").val();
